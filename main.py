...
MOTION_LABELS = {0: "hareketsiz", 1: "hareketli", -1: "hareket_durumu_yok"}
LANDING_LABELS = {0: "uygun_degil", 1: "uygun", -1: "inis_alani_degil"}
...
# =========================================================
# SINIFLAR (CameraMotionEstimator ve SimpleTracker aynı)
# =========================================================
...
# =========================================================
# ANA MANTIKSAL FONKSİYONLAR (get_motion_status ve get_landing_status aynı)
# =========================================================
def get_motion_status(class_id, track_id, box, track_history, cam_dx, cam_dy):
    ...
    return 1 if math.hypot(comp_dx, comp_dy) >= MOTION_PIXEL_THRESHOLD else 0

def get_landing_status(class_id, area_box, all_detections):
    if class_id not in [2, 3]: return -1
    for det in all_detections:
        if det["class_id"] in [0, 1] and point_in_box(center_of_box(det["box"]), area_box): return 0
        if det["class_id"] in [0, 1] and overlap_ratio_on_other(area_box, det["box"]) >= 0.30: return 0
    return 1
...
def main():
    ...
    # [DEĞİŞİKLİK]: Liste yerine sözlük kullanıyoruz
    final_results = {} 
    ...
    for idx, img_path in enumerate(image_files):
        ...
        # Frame için tespit listesini başlat
        final_results[f"frame_{frame_idx}"] = []
        ...
        if results[0].boxes is not None:
            for box, cls_id, conf in zip(results[0].boxes.xyxy.cpu().numpy(), results[0].boxes.cls.cpu().numpy(), results[0].boxes.conf.cpu().numpy()):
                detections.append({"class_id": int(cls_id), "class_name": CLASS_NAMES.get(int(cls_id)), "conf": float(conf), "box": [int(b) for b in box]})
        
        detections = tracker.update(detections)
        
        for det in detections:
            det["motion_status_id"] = get_motion_status(det["class_id"], det["track_id"], det["box"], track_history, cam_dx, cam_dy)
            det["motion_status_name"] = MOTION_LABELS[det["motion_status_id"]]
            det["landing_status_id"] = get_landing_status(det["class_id"], det["box"], detections)
            det["landing_status_name"] = LANDING_LABELS[det["landing_status_id"]]
            
            # [DEĞİŞİKLİK]: İlgili frame listesine ekle
            final_results[f"frame_{frame_idx}"].append(det)
